...
- 0 — Успешное выполнение (анализ завершён)
- 1 — Ошибка (директория не найдена, нет прав доступа и т.п.)

## 🎯 Заключение

dir_humanize — это пример того, как с помощью всего одной библиотеки можно значительно улучшить пользовательский опыт консольной утилиты. Вместо написания собственных сложных функций форматирования, проект использует готовые, локализованные и гибко настраиваемые решения от humanize.

Этот подход:

- Ускоряет разработку — не нужно изобретать велосипед.
- Повышает качество — готовые функции humanize хорошо протестированы и покрывают множество edge-кейсов.
- Делает приложение интернациональным — смена локали требует всего одной строки кода.

Проект демонстрирует, что создание профессиональных и удобных CLI-инструментов может быть быстрым, простым и даже увлекательным.
